    ...
    ws.on("message", function incomingGuess(guess) { // for a web socket connection made to the ws server by the client
        ...
        wss.clients.forEach((client) => { // for each client connnected via web socket
            if (client.readyState == WebSocket.OPEN) { // check that client's connection is open
                if (guess.startsWith(INCOMING_GUESS)) { // make sure the client has the correct string at the beginning of the message
                    console.log("sending message to others");
                    // const guessAccepted = guess.replace(INCO, ""); // remove start of string - we know that it's from the client now, and we don't need this showing up in the guesses
                    client.send(guess); // send the guess to the client
                }
            }
        })
